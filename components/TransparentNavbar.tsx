...
export function TransparentNavbar() {
    ...
    return (
        <header className="bg-transparent backdrop-blur-sm relative top-0 z-[1000] w-full">
            <div className="flex items-center justify-between py-4 px-4 md:px-6 max-w-[1280px] mx-auto">
                {/* Logo */}
                <Link href="/" className="text-2xl font-bold text-white no-underline transition-colors duration-[200ms] hover:text-white/80 [text-shadow:0_2px_8px_rgba(0,0,0,0.5)]">
                    Skillverge
                </Link>
                ...
                <nav className="hidden md:flex items-center gap-8">
                    <ul className="flex items-center gap-6 list-none m-0 p-0">
                        {navLinks.map((link) => (
                            <li key={link.name}>
                                <Link
                                    href={link.href}
                                    className={`text-base font-medium no-underline py-2 px-3 rounded-lg transition-all duration-[200ms] relative [text-shadow:0_2px_4px_rgba(0,0,0,0.3)] ${isActive(link.href)
                                        ? "text-white font-semibold after:content-[''] after:absolute after:bottom-0 after:left-3 after:right-3 after:h-0.5 after:bg-white"
                    ...
                    <div className="flex items-center gap-4">
                        {/* Cart Icon */}
                        <Link href="/cart" className="relative p-2 text-white/90 hover:text-white transition-colors [text-shadow:0_2px_4px_rgba(0,0,0,0.3)]">
                            <ShoppingCart className="w-6 h-6" />
                            {cartCount > 0 && (
                                <span className="absolute -top-1 -right-1 bg-[#FF4757] text-white text-xs font-bold w-5 h-5 flex items-center justify-center rounded-full border-2 border-white shadow-lg">
                                    {cartCount}
                                </span>
                            )}
                        </Link>
                        ...
                        {isAuthenticated ? (
                            <div className="relative group">
                                <button className="flex items-center gap-2 focus:outline-none">
                                    <div className="w-8 h-8 rounded-full bg-white text-[#2D6DF6] flex items-center justify-center font-bold text-sm shadow-lg">
                            ...
                            <>
                                <Link
                                    href="/login"
                                    className="inline-block py-2 px-5 text-sm font-medium text-center rounded-none transition-all duration-[300ms] cursor-pointer no-underline leading-[1.5] bg-transparent text-white border-2 border-white hover:bg-white hover:text-[#2D6DF6] hover:-translate-y-0.5 shadow-lg"
                                >
                                    Login
                                </Link>
                                <Link
                                    href="/signup"
                                    className="inline-block py-2 px-5 text-sm font-medium text-center rounded-none transition-all duration-[300ms] cursor-pointer no-underline leading-[1.5] bg-white text-[#2D6DF6] hover:bg-white/90 hover:-translate-y-0.5 shadow-lg"
                                >
                                    Sign Up
                                </Link>
                            </>
                ...
                <div className="flex items-center gap-4 md:hidden">
                    ...
                    >
                        <span className={`relative w-6 h-0.5 bg-white transition-all duration-[300ms] ${isOpen ? 'bg-transparent' : ''} before:content-[''] before:absolute before:w-6 before:h-0.5 before:bg-white before:transition-all before:duration-[300ms] ${isOpen ? 'before:top-0 before:rotate-45' : 'before:-top-2'} after:content-[''] after:absolute after:w-6 after:h-0.5 after:bg-white after:transition-all after:duration-[300ms] ${isOpen ? 'after:bottom-0 after:-rotate-45' : 'after:-bottom-2'}`}></span>
                    </button>
                </div>
            </div>
